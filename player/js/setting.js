var YZM = {
    ...
    'start_ec': function() {
        let html = '<table width="100%" height="100%" align="center" style="background: #222;color: #fff;"><tbody><tr><td align="center"><h1>解析失败，请重试或切换解析线路~<font size="2"></font><p></p></h1></td></tr></tbody></table>';
        if(config.url == ''){
            $('body').html(html);
        }
        if (config.group_x.indexOf(config.group) >= 0 && config.group != '') {
            if (config.state == '1') {
                YZM.MYad.vod(config.ads.vod.time, config.ads.vod.link);
            }else{
                YZM.MYad.pic(config.ads.pic.link, config.ads.pic.time, config.ads.pic.img);
            }
        } else {
            YZM.play(config.url);
        }
    ...
    'play': function(url) {
        ...
        if(config.marquee.length > 2){
            $(".marquee").html("<marquee>"+config.marquee+"</marquee>");
        }
        if (config.title.length > 2) {
            $("#vodtitle").html(config.title + '  ' + config.sid);
        }
    },
    'dmid': function() {//获取弹幕id
        if (config.sid.length>1) {
            YZM.id = config.sid
        }else{
            YZM.id = config.id
        }
    },
    'load': function() {//加载中显示的
        setTimeout(function() {
            $("#link1").fadeIn();
        }, 100);
        setTimeout(function() {
    ...
    'def': function() {
        ...
        YZM.dp.on('pause', function() {
            YZM.MYad.pause.play(config.ads.pause.link, config.ads.pause.img);
        });
    ...
    'video': {
                    ...
                    if (s > t && up.trysee>0) {
                        ...
                        layer.confirm(up.trysee + "分钟试看已结束，请登录继续播放完整视频", {
                            ...
                            yes: function(index, layero) {
                                top.location.href = config.login;
                            },
                            btn2: function(index, layero) {
                                top.location.href = config.reg;
                            }
                        });
                    }
                }, 1000);
        ...
        'con_play': function() {
            if (config.danmuon==1) {
                ...
                var conplayer =
                    ` <e>已播放至${YZM.ctime}，继续上次播放？</e><d class="conplay-jump">是 <i id="num">${config.waittime}</i>s</d><d class="conplaying">否</d>`
    ...
    'danmu': {
        ...
        'post_r': function(a, b, c, d, type) {
            $.ajax({
                type: "get",
                url: config.api + 'report?cid=' + d + '&user=' + a + '&type=' + type + '&title=' + b + '&text=' + c,
                cache: false,
                dataType: 'json',
                beforeSend: function() {},
    ...
    'endedHandler': function() {
        ...
        if (config.next.length>2) {
            ...
            setTimeout(function() {
                //播放下一集，防止资源泄露我们采用跳转下一个播放地址YZM.video.next();
                parent.window.location.href=config.next;
            }, 5 * 1000);
        } else {
    ...
    'hello':function(a) {
        var key  = CryptoJS.enc.Utf8.parse('abcdefghijklmn11');
        var iv   = CryptoJS.enc.Utf8.parse('abcdefghijklmn12');
